...
from .worldmap import WorldObject, ServerMessage
...
# "sends" a message to the server by adding a serverMessage object to the worldmap which is shared in
# sharedmap.py
class ServerSend(StateNode):
    ...
    def start(self, event=None):
        super().start()
        # add it to the world_map
        newMsgObj = ServerMessage(cozmo_id = self.robot.aruco_id, message = self.msg)
        self.robot.world.world_map.objects['Message-'+str(time.time())] = newMsgObj
        self.post_completion()


# Gets all messages in the world map, prints them, and posts them  
class GetMessages(StateNode):
    def start(self, event=None):
        super().start()
        messages = []
        ...
        self.post_data(messages)
        self.post_completion()

# Gets all messages in the world map, prints them, and posts them  
class ClientGetMessages(StateNode):
    def start(self, event=None):
        super().start()
        messages = []
        for key, value in self.robot.world.world_map.shared_objects.items():
            if isinstance(key, str) and 'Message' in key:# and value.id != self.robot.aruco_id:
                print(value)
                messages.append(value.msg)
        self.post_data(messages)
...
class ServerGetMessages(StateNode):
    def start(self, event=None):
        super().start()
        messages = []
        for key, value in self.robot.server.foreign_objects.items():
            if isinstance(key, str) and 'Message' in key:# and value.id != self.robot.aruco_id:
                print(value)
                messages.append(value.msg)
        self.post_data(messages)
