...
class StateMachineProgram(StateNode):
    def __init__(self,
        ...
        if hasattr(self.robot.world, 'is_server'):
            pass
        else:
    ...
    def handle_observed_motion(self, evt, **kwargs):
    ...
    def motionAnnotate(self,image,scale):
        for evt in self.queue:
            if evt.has_right_movement:
                cv2.circle(image, (evt.right_img_pos.x*scale, evt.right_img_pos.y*scale),10,(0,0,255),-1)
            if evt.has_left_movement:
                cv2.circle(image, (evt.left_img_pos.x*scale,evt.left_img_pos.y*scale),10,(0,0,255),-1)
            if evt.has_top_movement:
                cv2.circle(image, (evt.top_img_pos.x*scale,evt.top_img_pos.y*scale),10,(0,0,255),-1)
        return image        

    def user_annotate(self,image): return image
    ...
    def process_image(self,event,**kwargs):
        ...
        for i in range(len(self.queue)): 
            ...
            if self.robot.last_image_robot_timestamp - evt.timestamp > 2000:
        ...
        if self.force_annotation or self.windowName is not None:
            ...
            annotated_im = self.motionAnnotate(annotated_im,scale)
